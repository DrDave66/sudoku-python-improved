...
class Stats:
    ...
    def get_stats_profile(self):
        ...
        func_profiles = {}
        stats_profile = StatsProfile(total_tt, total_calls, func_profiles)
    ...
    def print_call_heading(self, name_size, column_title):
        ...
        if subheader:
            print(
                " " * name_size + "               ncalls    tottime   %tottime   cumtime  %cumtime    file:line:function",
                file=self.stream)
        ...
        for func in clist:
            ...
            if isinstance(value, tuple):
                ...
                if nc != cc:
                    ...
                    substats = '%d' % (nc,)
                substats = '      %s  %s  %s  %s  %s    %s' % (substats.rjust(12 + 2 * len(indent)),
                                                               f8(tt), f8p(tt / self.total_tt),
                                                               f8(ct), f8p(ct / self.total_tt), name)
    ...
    def print_title(self):
        print('      ncalls    tottime   %tottime   percall    cumtime  %cumtime    percall ', end=' ',
              file=self.stream)
    ...
    def print_line(self, func):  # hack: should print percentages
        ...
        print(f8p(tt / self.total_tt), end='  ', file=self.stream)
        if nc == 0:
            print(' ' * 8, end='  ', file=self.stream)
        else:
            print(f8(tt / nc), end='  ', file=self.stream)  # per call
        print(f8(ct), end='  ', file=self.stream)
        print(f8p(ct / self.total_tt), end='  ', file=self.stream)
        if cc == 0:
            print(' ' * 8, end='  ', file=self.stream)
        else:
            print(f8(ct / cc), end='  ', file=self.stream)  # per call
...
def f8(x: float) -> str:  # time will be passed in
    if x >= 1:
        ...
        return f"{x * 1000:8.3f}m"
    elif x >= 0.000001:
        return f"{x * 1000000:8.3f}u"
    else:
...
if __name__ == '__main__':
    ...
    class MyProfileBrowser(cmd.Cmd):
        ...
        def do_dump(self, line):
            ...
            self.output(f"Dumping stats to file {line}")
            return self.generic('write_stats_to_file', line)

        def do_clear(self, line):
            """Clears the current stats"""
            self.output(f"Clearing stats")
            self.prompt = ">> "
            self.files = []
            return self.generic('clear_stats', line)
        ...
        def do_add(self, line):
            ...
            if self.stats:
                try:
                    ...
                    self.output(f"Added {line} to stats")
                except OSError as e:
                    print("\t\t\tFailed to load statistics for %s: %s" % (line, e), file=self.stream)
            else:
                print("\t\t\tNo statistics object is loaded.", file=self.stream)
            if len(self.files) > 1:
                self.prompt = "multiple files  >> "
            elif len(self.files) == 1:
                self.prompt = f"{self.files[0]}  >> "
            else:
                self.prompt = ">> "
        ...
        def do_EOF(self, line):
            ...
            self.output(f"Exiting the profile browser")
            print("", file=self.stream)
            return 1

        def do_quit(self, line):
            """Leave the profile browser."""
            self.output("Exiting the profile browser")
            return 1
        ...
        def do_read(self, line):
            ...
            if line:
                ...
                line = self.files[0]
                self.do_read(line)
                self.output(f"Re-read file {line}")
            else:
        ...
        def do_reverse(self, line):
            ...
            if self.stats:
                ...
                self.output("Reversed sort order")
            else:
                print("\t\t\tNo statistics object is loaded.", file=self.stream)
            return 0
        ...
        def do_sort(self, line):
            ...
            self.output(f"Sorted stats by {line}")
            return 0
        ...
        def do_strip(self, line):
            ...
            if self.stats:
                ...
                self.output("Stripped directory names")
            else:
                print("\t\t\tNo statistics object is loaded.", file=self.stream)

        def do_list(self, line):
            'Lists the current stats files being used'
            if len(self.files) == 0:
    ...
    try:
        ...
        print("Welcome to my profile statistics browser.", file=browser.stream)
        browser.cmdloop()
